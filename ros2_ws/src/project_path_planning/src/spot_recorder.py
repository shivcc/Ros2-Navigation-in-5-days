...
class SpotRecorder(Node):
    ...
    def save_spot_callback(self, request, response):
        if request.label.lower() == 'end':
            # Create properly formatted parameter data
            parameter_data = {
                'move_to_spot': {                # Node name that will use these parameters
                    'ros__parameters': {         # Required ROS2 parameters section
                        'spots': self.spots      # Actual spot data
                    }
                }
            }
            ...
            try:
                with open(file_path, 'w') as file:
                    yaml.dump(parameter_data, file, sort_keys=False)
